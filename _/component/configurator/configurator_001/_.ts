...
export default class 
{
   ...
   {
      const outer_container: HTMLElement | null = document.getElementById(outer_container_id);

      if(outer_container !== null)
      {
         const validated_outer_container: HTMLDivElement = outer_container as HTMLDivElement;

         const access_manager_manager_instance: access_manager = new access_manager(
            root_folder,
            page_name,
            component_id,
            component_class_name,
            validated_outer_container
         );
